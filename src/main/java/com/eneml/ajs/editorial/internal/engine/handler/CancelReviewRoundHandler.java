...
@Component
class CancelReviewRoundHandler extends AbstractDecisionHandler {

    @Override public DecisionType type() { return DecisionType.CANCEL_REVIEW_ROUND; }
    ...
    public DecisionOutcome decide(SubmissionSummary current, DecisionContext context) {
        ...
        return DecisionOutcome.stay(current.stage(), current.status())
                .withSideEffect(DecisionOutcome.SideEffect.CANCEL_LATEST_REVIEW_ROUND);
    }
}
