...
export interface UserMenuProps {
  ...
  /** Trigger variant: chip = compact top-bar link, badge = full-width sidebar dropdown. */
  variant?: "chip" | "badge";
  /** Sign-in CTA href when {@link user} is null. Used by both variants. */
  signInHref: string;
  signInLabel?: string;
  /** Chip-only: where the "Dashboard" link points when signed in. Defaults to {@link signInHref}. */
  dashboardHref?: string;
  dashboardLabel?: string;
  /** Badge-only: items shown above the sign-out action. */
  items?: UserMenuItem[];
  /** Badge-only: sign-out callback. Renders the destructive row when set. */
  onSignOut?: () => void;
  signOutLabel?: string;
}
